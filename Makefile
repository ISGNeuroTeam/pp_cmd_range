...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"range_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f range-*.tar.gz
	echo Create archive \"range-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../range-$(VERSION)-$(BRANCH).tar.gz range
...
range.tar.gz: build
	cd make_build; tar czf ../range.tar.gz range && rm -rf ../make_build
